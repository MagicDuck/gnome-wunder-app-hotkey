...
export default class HappyAppyHotkeyExtension extends Extension {
  ...
  focusOrLaunch(tuple) {
    ...
    let overall_first_time = 0;
    let overall_second_time = 0;
    let overall_first_win = null;
    let overall_second_win = null;
    ...
    for (let i = 0; i <= wins.length; i++) {
      ...
      if (mw) {
        ...
        if (userTime > overall_first_time) {
          overall_second_time = overall_first_time
          overall_second_win = overall_first_win

          overall_first_time = userTime
          overall_first_win = mw
        } else if (userTime < overall_first_time && userTime > overall_second_time) {
          overall_second_time = userTime
          overall_second_time = mw
        }
      }
    }
    ...
    if (appWindows.length > 0) {
      if (activeAppWindow) {
        ...
        if (appWindows.length == 1 && this.settings.get_boolean('hide-active') && overall_second_win) {
          // this.hide(activeAppWindow);
          // activeAppWindow.lower()
          this.activate(overall_second_win)
        }
        else {
